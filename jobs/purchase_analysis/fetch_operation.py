...
def table_structure_matches(table_name: str, sample_row: Dict[str, Any]) -> bool:
    """检查表结构是否匹配"""
...
async def main():
    ...
    try:
        ...
        # 创建或检查表结构
        create_table_if_needed(table_name, sample_row)
        
        # 插入所有数据到数据库
        logger.info(f"\n6. 正在插入所有数据到数据库...")
        insert_data_batch(table_name, data_list)
