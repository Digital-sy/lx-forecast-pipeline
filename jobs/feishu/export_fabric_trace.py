...
"""
290涤双磨 完整溯源导出脚本
输出：/tmp/fabric_290_trace.xlsx（英文名，方便scp下载）

用法：
  cd /opt/apps/pythondata && source venv/bin/activate
  python scripts/export_290_trace.py
"""
...
with db_cursor(dictionary=True) as cur:
    cur.execute("""
        SELECT
            ...
            ROUND(MAX(COALESCE(s_prev.月销量, 0)) /
                  DAY(LAST_DAY(DATE_SUB(CURDATE(), INTERVAL 1 MONTH))), 2) AS T减1月日均,
            MAX(COALESCE(s_curr.月销量, 0))                             AS T月已销量,
            ROUND(MAX(COALESCE(s_curr.月销量, 0)) /
                  NULLIF(DAY(CURDATE()) - 1, 0), 2)                    AS T月日均,
        ...
        LEFT JOIN (
            SELECT SKU, SUM(销量) AS 月销量
            FROM `销量统计_msku月度`
            WHERE 统计日期 = DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y-%m-01')
            GROUP BY SKU
        ) s_prev ON CONVERT(s_prev.SKU USING utf8mb4) = CONVERT(p.SKU USING utf8mb4)
        LEFT JOIN (
            SELECT SKU, SUM(销量) AS 月销量
            FROM `销量统计_msku月度`
            WHERE 统计日期 = DATE_FORMAT(CURDATE(), '%Y-%m-01')
            GROUP BY SKU
...
with db_cursor(dictionary=True) as cur:
    cur.execute("""
        ...
        INNER JOIN `面料核价表` fk
            ...
            AND CONVERT(fk.面料 USING utf8mb4) = '290涤双磨'
        WHERE LEFT(a.创建时间, 7) = DATE_FORMAT(NOW(), '%Y-%m')
...
print(f"  系统预测SKU溯源： {len(df2)} 行（每SKU每月一行，已聚合店铺）")
