...
颜色拆分支持两种口径：
A = 仅 SPU 的主面料拆具体颜色；其他面料只进入面料总量。
B = 只要 SPU 配置了该面料，就按当前 SKU 的最终飞书颜色拆分。
默认使用方案 B；方案 A 仅用于历史留档/对照。
...
async def build_final_rows(
    ...
    color_split_mode: str = "B",
) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    color_split_mode = str(color_split_mode or "B").strip().upper()
    if color_split_mode not in {"A", "B"}:
        raise ValueError(f"不支持的颜色拆分方案: {color_split_mode}；仅支持 A/B")
    ...
    def add_usage(sku: str, qty: int, target: str, delta: int | None = None) -> None:
        ...
        for fabric_name, usage_data in usage_by_spu.get(spu, ()):
            ...
            # 方案A：只有主面料拆具体颜色；方案B：所有已配置面料都按SKU颜色拆分。
            if (
                color_split_mode == "A"
                and primary_fabric_by_spu.get(spu) != fabric_name
            ):
                continue
    ...
    metrics = {
        ...
        "color_split_mode": color_split_mode,
        "color_split_policy": (
            "A: 仅主面料拆具体颜色"
            if color_split_mode == "A"
            else "B: SPU配置的所有目标面料均按SKU最终飞书颜色拆分"
        ),
...
def main() -> Path:
    ...
    parser.add_argument(
        ...
        choices=("A", "B", "a", "b"),
        default="B",
        help="颜色拆分方案：A=仅主面料拆色；B=所有已配置面料按SKU拆色（默认）",
    )
    ...
    return asyncio.run(
        run(
            ...
            color_split_mode=args.color_split_mode,
        )
    )
